    ...
    public static int Part1(AocInput input) =>
        input.Text
            ...
            .Aggregate((new Coordinate(0, 0), Direction.North), (seed, move) =>
            {
                ...
                var intDirection =
                    ((move.Turn == 'R' ? 1 : -1) + (int)direction + 4) % 4;
                var newDirection = (Direction)intDirection;
                return newDirection switch
                {
                    Direction.North => (
                        coordinate with { X = coordinate.X + move.Blocks },
                        newDirection),
                    Direction.East => (
                        coordinate with { Y = coordinate.Y - move.Blocks },
                        newDirection),
                    Direction.South => (
                        coordinate with { X = coordinate.X - move.Blocks },
                        newDirection),
        ...
        | (final => final.Item1)
        | (endCoordinate =>
            ManhattanDistance(new Coordinate(0, 0), endCoordinate));
