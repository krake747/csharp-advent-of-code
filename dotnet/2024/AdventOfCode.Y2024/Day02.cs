...
public sealed class Day02 : IAocDay<int>
{
    public static int Part1(AocInput input) =>
        input.Lines
        | (lines => Instructions(lines).Count(Monotonic));

    public static int Part2(AocInput input) =>
        input.Lines
        | (lines => Instructions(lines).Count(instructions => ProblemDampener(instructions).Any(Monotonic)));

    private static bool Monotonic(int[] instructions) =>
        instructions.Zip(instructions.Skip(1), (l, r) => new Pair(l, r)).ToArray()
        | (pairs => MonotonicIncreasing(pairs) || MonotonicDecreasing(pairs));
